...
import { TUTOR_ALEX_SYSTEM_PROMPT } from '@/utils/prompt';
...
export async function POST(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: 'Por favor, insira sua chave de API nas configurações ou defina a variável de ambiente GEMINI_API_KEY.',
        },
        { status: 400 }
      );
    }
    ...
    let systemInstruction = TUTOR_ALEX_SYSTEM_PROMPT;
    if (material && material.trim().length > 0) {
      systemInstruction += `\n\nIMPORTANTE: Use o seguinte material de apoio fornecido pelo aluno como base principal de conhecimentos para a explicação e perguntas de fixação:\n"""\n${material}\n"""`;
    }
    ...
    return NextResponse.json(
      {
        ...
        message: error.message || 'Ocorreu um erro ao processar a resposta do Tutor Alex.',
      },
      { status: 500 }
    );
  }
